import requests,execjs
from user import User
SecretKey="F4&JF5*13$W7+C96C=6E4@1E9B#EAEA&4C1J#7A*FT2(B9U!C5)F0%3L^0C4B4"
cookies = {
    'JSESSIONID': 'E8DA7FF504F90CF3EDCEA4ABA5D8C490',
}
headers = {
    'Accept': '*/*',
    'Accept-Language': 'zh-CN,zh;q=0.9,en;q=0.8,en-GB;q=0.7,en-US;q=0.6',
    'Cache-Control': 'no-cache',
    'Connection': 'keep-alive',
    'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
    'Origin': 'https://xkxt2.ecjtu.edu.cn',
    'Pragma': 'no-cache',
    'Referer': 'https://xkxt2.ecjtu.edu.cn/',
    'Sec-Fetch-Dest': 'empty',
    'Sec-Fetch-Mode': 'cors',
    'Sec-Fetch-Site': 'same-origin',
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/149.0.0.0 Safari/537.36 Edg/149.0.0.0',
    'X-Requested-With': 'XMLHttpRequest',
    'sec-ch-ua': '"Microsoft Edge";v="149", "Chromium";v="149", "Not)A;Brand";v="24"',
    'sec-ch-ua-mobile': '?0',
    'sec-ch-ua-platform': '"Windows"',
    # 'Cookie': 'JSESSIONID=E8DA7FF504F90CF3EDCEA4ABA5D8C490',
}
with open('1.js', 'r', encoding='utf-8') as f:
    js_code = f.read()
ctx = execjs.compile(js_code)
User1 = User()
data = {
    'username': ctx.call('encryptByDES', User1.user_name, SecretKey),
    'password': ctx.call('encryptByDES', User1.user_pwd, SecretKey),
    'code': '3473',
}
print(data)

response = requests.post('https://xkxt2.ecjtu.edu.cn/login/login_checkout.action', cookies=cookies, headers=headers, data=data)
print(response.text)